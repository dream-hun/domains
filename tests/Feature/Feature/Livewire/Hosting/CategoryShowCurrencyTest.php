...
use App\Livewire\Hosting\CategoryShow;
...
test('category show component shows prices in session currency', function (): void {
    ...
    $category->load(['plans.planPrices.currency', 'plans.planFeatures']);
    ...
    expect($html)->toContain('Shared Hosting')
        ->and($html)->toContain('Starter')
        ->and($html)->toContain('Starting at')
        ->and($html)->toContain('10');
});

test('category show component updates prices when currency changes', function (): void {
