                ...
                <div class="col-lg-12">
                    <div class="banner-area">
                        <div class="rts-hosting-banner rts-hosting-banner__content">
                            @php
                                $cheapestMonthlyPrice = $this->getCheapestMonthlyPrice();
                            @endphp
                            @if ($cheapestMonthlyPrice)
                                <span class="starting__price sal-animate" data-sal="slide-down" data-sal-delay="100"
                                    ...
                                    {{ $cheapestMonthlyPrice->getFormattedPrice('regular_price', $this->displayCurrencyForPrice($cheapestMonthlyPrice)) }}/mo</span>
                            @else
                                <span class="starting__price sal-animate" data-sal="slide-down" data-sal-delay="100"
                                    data-sal-duration="800">Contact for pricing</span>
                            @endif
        ...
        <div class="container">
                ...
                <div class="price__content open" id="monthly" style="">
                    <div class="row g-30 monthly">
                        @foreach ($plans as $plan)
                            @php
                                $monthlyPrice = $this->getPriceForCycle($plan, 'monthly');
                            @endphp
                            @if ($monthlyPrice)
                                <div class="col-lg-4 col-md-6 col-sm-6" wire:key="plan-{{ $plan->id }}-monthly">
                                    <div class="card-plan active">
                                        ...
                                        <h5 class="card-plan__price">
                                            {{ $monthlyPrice->getFormattedPrice('regular_price', $this->displayCurrencyForPrice($monthlyPrice)) }} <sub>/ month</sub>
                                        </h5>
                                        ...
                                        <p class="card-plan__renew-price">
                                            {{ $monthlyPrice->getFormattedPrice('renewal_price', $this->displayCurrencyForPrice($monthlyPrice)) }} /Month when you
                                            renew
                                        </p>
            ...
            <div class="price__content" id="yearly" style="display: none;">
                <div class="row g-30 yearly">
                    @foreach ($plans as $plan)
                        @php
                            $yearlyPrice = $this->getPriceForCycle($plan, 'annually');
                        @endphp
                        @if ($yearlyPrice)
                            <div class="col-lg-4 col-md-6 col-sm-6" wire:key="plan-{{ $plan->id }}-yearly">
                                <div class="card-plan active">
                                    @if ($plan->is_popular)
                                        <div class="popular-tag">most popular</div>
                                    @endif
                                    ...
                                    <p class="card-plan__desc">{{ $plan->description }}</p>
                                    @if ($yearlyPrice->discount_percentage ?? null)
                                        <div class="card-plan__offer">
                                            <span
                                                class="past-price">{{ $yearlyPrice->getFormattedPrice('regular_price', $this->displayCurrencyForPrice($yearlyPrice)) }}</span>
                                            <span class="offer-given">Save
                                                {{ $yearlyPrice->discount_percentage }}%</span>
                                        </div>
                                    @endif
                                    <h5 class="card-plan__price">
                                        {{ $yearlyPrice->getFormattedPrice('regular_price', $this->displayCurrencyForPrice($yearlyPrice)) }} <sub>/ Year</sub>
                                    </h5>
                                    <div class="card-plan__cartbtn">
                                        <a href="{{ route('hosting.configure', ['plan' => $plan->uuid, 'billing_cycle' => 'annually']) }}">add to cart</a>
                                    </div>
                                    <p class="card-plan__renew-price">
                                        {{ $yearlyPrice->getFormattedPrice('renewal_price', $this->displayCurrencyForPrice($yearlyPrice)) }} /Year when you renew
                                    </p>
                                    <div class="card-plan__feature">
                                        @php
                                            $allIncludedFeatures = $plan->planFeatures->where('is_included', true);
