<div class="card">
    ...
    <div class="card-body">
        ...
        @can('vps_snapshot_create')
            ...
            <form method="POST" action="{{ route('admin.vps.snapshots.store', $subscription) }}" class="mb-4">
                @csrf
                <div class="row">
                    <div class="col-md-4">
                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        @can('vps_backup_restore')
                                            <form method="POST" action="{{ route('admin.vps.snapshots.restore', [$subscription, $snapshot['snapshotId']]) }}" class="d-inline" onsubmit="return confirm('Restore this snapshot? The current state will be overwritten.')">
                                                @csrf
                                        ...
                                        @can('vps_snapshot_delete')
                                            <form method="POST" action="{{ route('admin.vps.snapshots.destroy', [$subscription, $snapshot['snapshotId']]) }}" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this snapshot?')">
                                                @csrf
                                                @method('DELETE')
