...
final class DomainController extends Controller
{
    ...
    public function ownerShipForm(Domain $domain): View
    {
        ...
        if ($domain->owner_id !== auth()->id() && ! auth()->user()->isAdmin()) {
            return back()->with('error', 'You are not authorized to assign owner for this domain.');
        }
    ...
    {
        if ($domain->owner_id !== auth()->id() && ! auth()->user()->isAdmin()) {
            return back()->with('error', 'You are not authorized to update nameservers for this domain.');
        }
    ...
    {
        if ($domain->owner_id !== auth()->id() && ! auth()->user()->isAdmin()) {
            return back()->with('error', 'You are not authorized to toggle lock for this domain.');
        }
    ...
    {
        if ($domain->owner_id !== auth()->id() && ! auth()->user()->isAdmin()) {
            return back()->with('error', 'You are not authorized to reactivate this domain.');
        }
